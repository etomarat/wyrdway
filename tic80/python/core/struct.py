...
class Struct:
    ...
    - `__slots__` можно использовать для линтера/IDE, но в рантайме оно не работает
      как в CPython (инстансы всё равно имеют `__dict__`), поэтому на него нельзя
      завязывать поведение.
