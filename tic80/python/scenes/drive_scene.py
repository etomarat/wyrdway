...
class DriveScene:
    ...
    def _draw_pursuer_screen_fx(self, logic: DriveLogic) -> None:
        ...
        if dots <= 0:
            if flash_n > 0.0:
                self._draw_pursuer_strike_meltdown(flash_n, 0)
            return
        seed = int(
            logic.road_s * 13.0
        ...
        while i < dots:
            ...
            i += 1
        if flash_n > 0.0:
            self._draw_pursuer_strike_meltdown(flash_n, seed)

    def _draw_pursuer_strike_meltdown(self, flash_n: float, seed_base: int) -> None:
        if flash_n <= 0.0:
            return
        ...
        # "Пропавшие" куски кадра: шахматные дыры/артефакты как у dying-GPU.
