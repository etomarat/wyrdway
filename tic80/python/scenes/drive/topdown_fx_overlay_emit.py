...
def topdown_fx_maybe_emit_transition_sparks(
    ...
    if logic.offroad == overlay._prev_offroad:
        return
    d = TUNING.DRIVE
    spd = logic.speed
    if spd > d.fx_dust_min_speed and overlay._offroad_transition_cooldown <= 0.0:
        ...
        overlay._offroad_transition_cooldown = float(TUNING.DRIVE.fx_transition_cooldown_seconds)
    overlay._prev_offroad = logic.offroad


def topdown_fx_maybe_emit_offroad_dust(
    overlay: TopdownFxOverlay,
    logic: DriveLogic,
    dt: float,
    pose: CarPose2D
    ...
    if not logic.offroad or logic.speed <= d.fx_dust_min_speed:
        return
    overlay._fx_spawn_accum_off_smoke += (d.fx_dust_rate_offroad * 0.65) * dt
    topdown_fx_emit_offroad_smoke_vand(overlay, overlay._fx_spawn_accum_off_smoke, pose)
    overlay._fx_spawn_accum_off_smoke -= int(overlay._fx_spawn_accum_off_smoke)


def topdown_fx_maybe_emit_exhaust_smoke(
    overlay: TopdownFxOverlay,
    logic: DriveLogic,
    dt: float,
    pose: CarPose2D
    ...
    if strength <= 0.0:
        return
    overlay._exhaust_strength = strength
    rate = d.fx_exhaust_rate * strength
    overlay._fx_spawn_accum_exhaust += rate * dt
    topdown_fx_emit_exhaust_smoke_vand(
        overlay,
        overlay._fx_spawn_accum_exhaust,
        strength,
        pose
    )
    overlay._fx_spawn_accum_exhaust -= int(overlay._fx_spawn_accum_exhaust)


def topdown_fx_emit_offroad_smoke_vand(
    overlay: TopdownFxOverlay,
    count_accum: float,
    pose: CarPose2D
) -> None:
    ...
    i = 0
    while i < n:
        r0 = overlay._next_fx_seed()
        r1 = overlay._next_fx_seed()
        ...
        overlay._offroad_smoke.spawn_dust_down_color(float(x_l), float(y_l), float(r), int(c))
        overlay._offroad_smoke.spawn_dust_down_color(float(x_r), float(y_r), float(r), int(c))
        ...
        overlay._offroad_smoke.spawn_dust_down_color(float(x_l2), float(y_l2), float(r2), int(c2))
        overlay._offroad_smoke.spawn_dust_down_color(float(x_r2), float(y_r2), float(r2), int(c2))
        i += 1


def topdown_fx_emit_exhaust_smoke_vand(
    overlay: TopdownFxOverlay,
    count_accum: float,
    strength: float,
    pose: CarPose2D
    ...
    while i < n:
        ...
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x1, y1, sr, c0, c1, 18)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x2, y2, sr * 0.85, c0, c1, 16)
        ...
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x3, y3, mr, c0, c1, mid_life)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x4, y4, mr * 0.95, c0, c1, mid_life - 2)
        ...
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x5, y5, tail_r, c0, c1, tail_life)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x6, y6, tail_r * 0.92, c0, c1, tail_life - 2)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x7, y7, tail_r * 0.88, c0, c1, tail_life - 4)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x8, y8, tail_r * 0.84, c0, c1, tail_life - 6)
        overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x9, y9, tail_r * 0.78, c0, c1, tail_life - 8)
        if (r & 1) == 0:
            x10, y10 = pose.local_to_screen(base_x + (u - 0.5) * 2.8, tail_back + 5.4)
            overlay._exhaust_smoke.spawn_dust_down_two_tone_life(x10, y10, tail_r * 0.72, c0, c1, tail_life - 10)

        i += 1


def topdown_fx_emit_offroad_transition_sparks(
    overlay: TopdownFxOverlay,
    ...
    boundary_sign = overlay._offroad_side_sign
    ...
    dir_x, dir_y, cross = topdown_fx_edge_spark_dir(
        overlay, road, logic, proj, dir_sign, entering_offroad
    )
...
def topdown_fx_edge_spark_burst(
    overlay: TopdownFxOverlay,
    base_x: float,
    base_y: float,
    dir_x: float,
