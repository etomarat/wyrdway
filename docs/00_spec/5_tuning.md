...
- `data.tuning.drive.visual` (камера/рендер top-down)
- `data.tuning.drive.fx`
- `data.tuning.drive.objects`
- `data.tuning.drive.debug` (debug vectors/hitboxes/telemetry)

Версия тюнинга:
- `TUNING.tuning_version` — целое число, поднимается при изменении значений баланса.
- `tuning_version` сохраняется в сейв профиля и используется для предупреждения о “tuning mismatch”.
...
- **GameState / Profile / RunState** — текущее состояние игры и рана (см. `00_spec/3_runstate.md`). TUNING не хранит состояние.
...
- **Profile/RunState** — текущее состояние (внутри `GameState`).
...
Текущие домены (as-is) фактически группируются как:
- `CORE`, `DEBUG`, `PROFILE`, `POI`, `DRIVE.*`
...
### 11.3 Методика тюнинга экономики и апгрейдов
Для route/garage progression баланс не настраивается хаотичным набором “ещё одного забега”.

Базовый порядок:
1) сначала фиксируем baseline **без апгрейдов**;
2) затем настраиваем обычные награды POI и `checkpoint cache`;
3) затем настраиваем эффекты `Level 1`;
4) только после этого — цены `Level 2+`;
5) и только в самом конце — `Level 3`, если они вообще нужны.

Правило:
- не крутить одновременно и награды, и цены, и силу апгрейдов;
- за одну итерацию менять только **один слой** экономики.

#### Baseline-прогон
На baseline без апгрейдов проверяем:
- проходит ли средний игрок первый сегмент стабильно;
- доходит ли хороший игрок до второго-третьего сегмента;
- не превращается ли ран в repair tax после одной-двух ошибок.

#### Ограничение текущего процесса
Пока у проекта нет внешних плейтестеров, рабочий baseline строится в первую очередь вокруг **хорошего игрока**.

Это значит:
- ранний баланс не обещает идеальный onboarding для “среднего первого игрока” уже на первом проходе тюнинга;
- сначала настраиваем честный и интересный loop для уверенной игры;
- после появления внешних плейтестов отдельно проверяем, не слишком ли harsh early game для менее опытного игрока.

#### Тестовые билды
После baseline прогоняем как минимум три понятных сборки:
- `survival` — защита и выживаемость;
- `economy` — больше `scrap/fuel`;
- `utility` — бак и ремонт.

Смысл:
- не смотреть только “приятно / неприятно”;
- сравнивать, не ломает ли одна ветка прогрессию относительно других.

#### Что измерять в плейтесте
Минимальный набор метрик:
- сколько `scrap` в среднем довозится за сегмент;
- сколько `fuel` в среднем довозится за сегмент;
- сколько `scrap` уходит на ремонт;
- после скольких успешных сегментов открывается первый `Level 2`;
- как сильно `Level 1` меняет выживаемость или экономику;
- не становится ли одна сборка явно доминирующей.

#### Как трактовать результаты
- если игрок тонет без ресурсов даже при хорошей игре, сначала поднимаем доходность, а не режем цены;
- если `Level 1` почти не ощущается, сначала усиливаем эффект, а не удешевляем `Level 2`;
- если `economy build` слишком быстро разгоняет прогрессию, режем именно его коэффициенты, а не всю экономику целиком;
- если рост силы ощущается плоским, сначала проверяем базовые значения угрозы, а уже потом добавляем новые уровни.

#### Практическое правило
Для ближайшего релиза баланс route/garage loop считается удачным, если:
- хороший run заметно ускоряет доступ к `Level 2+`;
- плохая игра не блокирует прогрессию мгновенно, но оставляет игрока беднее;
- `checkpoint cache` страхует от полной нищеты, но не заменяет мастерство;
- разные ветки апгрейдов дают разные ощущения, а не одну и ту же выгоду разными словами.
