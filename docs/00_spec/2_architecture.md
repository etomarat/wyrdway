...
- **Обычная игра** (вертикальная петля M1): `IS_DRIVE_PLAYTEST = False` (значение по умолчанию).
- **DRIVE плейтест** (для тюнинга управления/камеры): `IS_DRIVE_PLAYTEST = True` (включается вручную).
...
- угрозы: обычные дорожные threats выключены, но активна погоня преследователя (если не playtest)
...
Режим включается флагом `IS_DRIVE_PLAYTEST = True` (в `tic80/python/main.py`; по умолчанию там `False`) и делает цикл для быстрого тюнинга:
