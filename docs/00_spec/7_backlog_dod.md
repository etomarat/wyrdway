...
- M1 закрыт (см. `docs/10_milestones/m1/m1_report.md`).
- DRIVE уже ушёл дальше M1: world-space модель и тюнинг (m1.5) и серия экспериментов по читаемости (m1.6).
...
2) **Сцены изолированы**: сцена владеет своим UI/логикой и общается через `GameState` (Profile/RunState).
...
**Расширяемость:**
- Profile/RunState используются как контракт; сцены не лезут в чужие данные напрямую.
