...
## Важное про экспорт (win/html)

Экспорт (`export win` / `export html`) может запускаться на **другом рантайме**, чем ваш
локальный редактор TIC-80 (и часто это более старая сборка). Из-за этого возможны
ошибки, которые **не воспроизводятся** при обычном запуске из редактора.

Практические правила для проекта:
- Для артефактов, которые нужно запускать вне редактора, используем `run_tic80_python.bat dist`
  (он прогоняет `tools/tic80_safe_postprocess.py` и убирает проблемный синтаксис).
- Для рисования линий используем `tic80/python/core/gfx.py:gfx_line`, а не прямой `line()`:
  в некоторых экспортированных плеерах `line()` вызывает `_line(int, ...)`, но `_line`
  ожидает float и падает.
...
- Не полагаться на `__setattr__`: присваивания атрибутов выглядят как прямые записи в `__dict__` (хуки `__setattr__` могут не вызываться).

Работает (по итогам прогона):
- `property`.
- Setter у `property` (значит это рабочий способ перехватить запись, в отличие от `__setattr__`).
...
## Практический вывод: `Struct` вместо `__slots__` / `__setattr__`

В PocketPy/TIC-80 присваивания атрибутов выглядят как прямые записи в `__dict__` и могут **не вызывать `__setattr__`**. Поэтому:
- Нельзя строить “строгие” модели данных, которые надеются на `__setattr__` для валидации или записи в другой storage.
- Если хочется хранить данные компактно/явно (например, в list `_v`) и при этом корректно обрабатывать `obj.x = ...`, рабочий путь — **`property` с setter**.

Заготовка под такой подход добавлена как экспериментальный helper: `tic80/python/core/struct.py` (класс `Struct`).
