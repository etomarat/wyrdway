...
- `__slots__` (в pocketpy может отсутствовать или игнорироваться; не использовать как рантайм‑контракт).
...
- `__slots__`: “есть”, но не работает как в CPython (инстансы всё равно имеют `__dict__`).
  Можно использовать для линтера/IDE, но не завязывать логику на отсутствие `__dict__`.
