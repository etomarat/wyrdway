...
* [x] Загрузка Profile при старте игры.
* [x] Сохранение Profile после действий в гараже (ремонт/апгрейды).
* [x] Сохранение Profile перед стартом рана.
* [x] Сохранение Profile после применения результата (RESULT → GARAGE).
* [x] Подтверждено: прогресс рана не сохраняем.
...
* [x] Оверлей показывает: текущая сцена, seed, узел, HP/топливо/таймер POI/scrap.
* [x] Переключатель on/off.
...
* [x] Пройти петлю 10 раз подряд без краша.
* [x] Проверить провал: смерть/таймер/штраф → гараж.
* [x] Проверить, что награды/штрафы не применяются дважды.
* [x] Проверить, что TUNING реально меняет ощущения (скорость/расход/таймер) без правки логики.
