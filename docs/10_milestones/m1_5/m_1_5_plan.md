...
6.4. Рендер зон (boost pads)

Zone/BoostPad: подсветка участка на дороге (полоса/мерцание) на диапазоне глубины.
...
Контекст: в top-down (и потенциально в cockpit) мы можем рисовать машину одним
...
- [ ] Zone/BoostPad можно распознать и осознанно использовать (ускориться/стабилизироваться).
